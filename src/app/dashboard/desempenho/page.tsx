    ...
    <div className="min-h-screen bg-bg flex">
      ...
      <aside className="w-64 bg-bg border-r border-border/60 flex flex-col fixed h-full z-10">
        ...
        <nav className="flex flex-col gap-0.5 px-3 flex-1 pt-1">
          ...
          <SidebarLink href="/dashboard/desempenho" icon={BarChart3} label="Desempenho" active />
          ...
          <section>
            ...
            <div className="grid grid-cols-3 gap-4">
              <OverviewCard
                ...
                value={stats.questoes_total.toLocaleString('pt-BR')}
                icon="📝"
              />
              <OverviewCard
                label="Taxa de acerto"
                value={totalTotal > 0 ? `${taxaAcerto}%` : '—'}
                icon="🎯"
                corClasse={totalTotal > 0 ? corTexto(taxaAcerto) : 'text-text-faint'}
              />
              <OverviewCard
              ...
              <OverviewCard
                ...
                value={`${stats.sequencia_dias} dia${stats.sequencia_dias !== 1 ? 's' : ''}`}
                icon="🔥"
              />
              <OverviewCard
                label="Recorde de sequência"
                value={`${stats.sequencia_maxima ?? 0} dia${(stats.sequencia_maxima ?? 0) !== 1 ? 's' : ''}`}
                icon="⚡"
              />
              <OverviewCard
                  ...
                  <div key={mat}>
                    ...
                    <div className="h-2 bg-bg-hover rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${p}%`, backgroundColor: corHex(p) }}
                      />
                    </div>
            ...
            {maisFraco && maisFraco.t >= 5 && maisFraco.p < 70 && (
              <div className="mt-4 bg-brand-soft border border-brand/20 rounded-2xl p-5">
                ...
                <p className="text-text text-sm leading-relaxed">
                  ...
                  <strong>{maisFraco.mat}</strong>.
                  Recomendamos revisar os conteúdos desta matéria usando a{' '}
                  <Link href="/dashboard/tutora" className="text-brand underline underline-offset-2 hover:no-underline">
                    IA Tutora
                  </Link>{' '}
                  ou gerar um{' '}
                  <Link href="/dashboard/resumos" className="text-brand underline underline-offset-2 hover:no-underline">
                    resumo focado
                  </Link>{' '}
