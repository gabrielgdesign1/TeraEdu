...
import {
  ...
  BarChart3, Calendar, Sun, Moon, Settings, Send, Plus
} from 'lucide-react'
...
export default function IATutora() {
  ...
  async function enviar(texto?: string) {
    ...
    try {
      ...
      setMensagens([...lista, { role: 'assistant', content: dados.resposta }])
    } catch {
      setMensagens([...lista, { role: 'assistant', content: 'Erro ao conectar. Tente novamente.' }])
    }
    setLoading(false)
  }
  ...
  return (
    <div className="min-h-screen bg-bg flex">
      ...
      <main className="flex-1 ml-64 flex flex-col h-screen overflow-hidden">
        ...
        {vazio ? (
          /* ── Tela vazia estilo Gemini ── */
          <div className="flex-1 flex flex-col items-center justify-center px-6 pb-10">
            <h1 className="text-text text-4xl font-bold tracking-tight mb-2 text-center">
              Olá{primeiroNome ? `, ${primeiroNome}` : ''}
            </h1>
            <p className="text-text-muted text-xl mb-10 text-center">
              Sobre o que quer aprender hoje?
            </p>
            ...
            <p className="text-text-faint text-xs mt-3">
              TeraEdu IA pode cometer erros. Sempre verifique informações importantes.
            </p>
          </div>
        ) : (
          <>
            ...
            <div className="px-4 pb-5 pt-2">
              <div className="max-w-2xl mx-auto">
                {inputBar}
                <p className="text-text-faint text-xs mt-2.5 text-center">
                  TeraEdu IA pode cometer erros. Sempre verifique informações importantes.
                </p>
              </div>
            </div>
